...
function CardFan({ inView }) {
  return (
    <div className={styles.fanWrap}>
      {FAN_CARDS.map((src, i) => {
        const half = Math.floor(FAN_CARDS.length / 2)
        const offset = i - half
        ...
        return (
          <motion.div
            ...
            animate={inView ? { opacity: 1, y: -ty, rotate, x: tx, scale: 1 } : {}}
            transition={{ delay: 0.2 + i * 0.08, duration: 0.7, type: 'spring', stiffness: 120, damping: 16 }}
            whileHover={{ y: -ty - 20, scale: 1.12, rotate: 0, zIndex: 20, transition: { type: 'spring', stiffness: 300, damping: 20 } }}
          >
...
function RippleButton({ children, onClick }) {
  ...
  return (
    <motion.button
      ...
      onClick={handleClick}
      whileHover={{ scale: 1.05, boxShadow: '0 20px 60px rgba(245, 64, 87, 0.5), 0 0 0 3px rgba(245, 64, 87, 0.15)' }}
      whileTap={{ scale: 0.97 }}
    >
      {ripples.map((r) => (
        <motion.span
      ...
      <div className={styles.inner}>
        ...
        >
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.04, backgroundColor: 'rgba(245, 64, 87, 0.06)' }}
            whileTap={{ scale: 0.97 }}
          >
            See Examples
          </motion.button>
        ...
        >
          <div className={styles.trustAvatars}>
            {['#F54057', '#FDA71E', '#2BBFA4', '#6C63FF'].map((c, i) => (
              <div key={i} className={styles.trustAvatar} style={{ background: c, zIndex: 4 - i }} />
            ))}
          </div>
